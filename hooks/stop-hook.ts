...
/**
 * Stop Hook - Voice notifications and tab titles
 *
...
import { EventBus } from './kai/event-bus';
import { securityMiddleware } from './kai/security';
import { logHook, getPAIDir } from './kai/shared';
...
function setTerminalTabTitle(title: string): void {
  // DISABLED: These ANSI escape sequences corrupt output in VSCode integrated terminal
  // The escape codes interfere with text rendering, causing character corruption
  // Re-enable only for native terminals that properly support these sequences
...
try {
  ...
  logHook('stop-hook', 'Could not load voices.json, using fallback config', 'warn');
  VOICE_CONFIG = {
    default_rate: 175,
    voices: {
...
async function main() {
  ...
  logHook('stop-hook', `Triggered at ${timestamp}`, 'debug');

  try {
    // Parse event using kai utilities
    const event = await EventBus.parseEvent('Stop');

    // Security validation
    if (!(await securityMiddleware(event, 'stop-hook'))) {
      logHook('stop-hook', 'Security validation failed', 'error');
      process.exit(0);
    }

    // Load transcript using kai utilities
    if (!event.transcript) {
      await EventBus.loadTranscript(event);
    }

    if (!event.transcript) {
      logHook('stop-hook', 'No transcript available', 'warn');
      process.exit(0);
    }

    logHook('stop-hook', `Transcript loaded: ${event.transcript.split('\n').length} lines`, 'debug');

    // Parse transcript using kai utilities
    const entries = EventBus.parseTranscript(event.transcript);

    // Get last user query using kai utilities
    const lastUserQuery = EventBus.getLastUserMessage(entries);
    ...
    if (lastAssistantMessage) {
      ...
      const customCompletedMatch = lastAssistantMessage.match(/🗣️\s*CUSTOM\s+COMPLETED:\s*(.+?)(?:\n|$)/im);
      ...
      if (customCompletedMatch) {
        ...
        if (customText && wordCount <= 8) {
          ...
          logHook('stop-hook', `KAI CUSTOM VOICE: ${message}`, 'debug');
        } else {
          // Custom completed too long, fall back to regular COMPLETED
          const completedMatch = lastAssistantMessage.match(/🎯\s*COMPLETED:\s*(.+?)(?:\n|$)/im);
          if (completedMatch) {
            let completedText = completedMatch[1].trim();
            message = generateIntelligentResponse(lastUserQuery, lastAssistantMessage, completedText);
            logHook('stop-hook', `KAI FALLBACK (custom too long): ${message}`, 'debug');
          }
        }
      } else if (!isAgentTask) {
        ...
        if (completedMatch) {
          ...
          logHook('stop-hook', `KAI INTELLIGENT: ${message}`, 'debug');
        } else {
          // No COMPLETED line found - don't send anything
          logHook('stop-hook', 'No COMPLETED line found', 'warn');
        }
      }
    }
    ...
    if (!message && isAgentTask && taskResult) {
      ...
      if (customCompletedMatch) {
        ...
        if (customText && wordCount <= 8) {
          ...
          logHook('stop-hook', `AGENT CUSTOM VOICE (fallback): ${message}`, 'debug');
        } else {
          // Custom completed too long, fall back to regular COMPLETED
          ...
          if (completedMatch) {
            ...
            voiceConfig = VOICE_CONFIG.voices[agentType.toLowerCase()] || VOICE_CONFIG.voices.kai;
            logHook('stop-hook', `AGENT FALLBACK (custom too long): ${message}`, 'debug');
          }
        }
      } else {
        ...
        if (completedMatch) {
          ...
          logHook('stop-hook', `AGENT INTELLIGENT (fallback): ${message}`, 'debug');
        }
      }
    }
    ...
    if (message) {
      ...
      logHook('stop-hook', `Voice notification sent: "${message}" with voice: ${voiceConfig.voice_name} at ${voiceConfig.rate_wpm} wpm (${voiceConfig.rate_multiplier}x)`, 'debug');
    }
    ...
    if (tabTitle) {
      try {
        ...
        logHook('stop-hook', `Tab title set to: "${tabTitle}"`, 'debug');
      } catch (e) {
        logHook('stop-hook', `Failed to set tab title: ${e}`, 'error');
      }
    }

    logHook('stop-hook', `User query: ${lastUserQuery || 'No query found'}`, 'debug');
    logHook('stop-hook', `Message: ${message || 'No completion message'}`, 'debug');
    ...
    logHook('stop-hook', `Completed successfully at ${new Date().toISOString()}`, 'debug');
  } catch (error) {
    logHook('stop-hook', `Error: ${error}`, 'error');
  }
}
